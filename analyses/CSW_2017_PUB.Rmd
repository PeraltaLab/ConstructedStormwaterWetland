...
date: "Last updated on 17 May, 2019"
#output:
  #pdf_document: null
  #editor_options: null
  #fig_caption: yes
...
ghg <- mutate(ghg, 
      #co2.ch4=ghg$co2cgmday/ghg$ch4cgm2day, #co2:ch4 ratio   
      ch4co2eq=ghg$ch4cgm2day*25, #methane carbon dioxide equivalents
      n2oco2eq=ghg$n2ongm2day*298 #nitrous oxide carbon dixode equivalents
      )
```

```{r plot formatting}
colman <- c("navy","darkgreen")
hydro_label <- c("PI", "SL")
season <- c("sp" = "spring", "sm" = "summer", "f" = "fall", "w" = "winter")
colseason <- c("blue","orange","brown","black")
```

```{r CH4 by season, echo=T, out.width="100%"}
#linear model for methane
i <- with(ghg, interaction(hydrology,season))
ghg.lm <- lm(ch4cgm2day~hydrology+season+i,data=ghg)
plot(ghg.lm)
anova(ghg.lm)
summary(ghg.lm)

t <- HSD.test(ghg.lm, "i", group=TRUE, alpha=0.0)
t
plot(t)

p <- ggplot(ghg, aes(x=season, y=ch4cgm2day, color=season, shape=hydrology)) + 
  stat_summary(fun.data=mean_cl_boot,size=1.25) +  
  scale_color_manual(name="Season", values=colseason, labels = season) +     
  scale_shape_manual(name="Hydrology", values=c(17,16), labels = hydro_label) +      
  scale_x_discrete(labels=season,limits=c("sp", "sm", "f", "w")) + 
  labs(x = "Season", y = (expression(paste("CH"[4], " (g C"," m"^-{2}," day"^-{1},")")))) +
  ggtitle("Seasonal GHG Fluxes") +
  #set Tukey groups from HSD.test above
  annotate("text",x=1.14,y=0.4, label="b") +
  annotate("text",x=2.14,y=0.4, label="b") +
  annotate("text",x=3.14,y=0.4, label="b") +
  annotate("text",x=4.14,y=0.4, label="b") +
  annotate("text",x=1.0,y=2.3, label="ab") +
  annotate("text",x=2.0,y=6.1, label="a") + 
  annotate("text",x=3.0,y=3.9, label="ab") + 
  annotate("text",x=4.0,y=2.8, label="ab") + 
  #set plot design
  theme_bw() + 
...
#linear model for methane
i <- with(ghg, interaction(hydrology,season))
ghg.lm <- lm(co2cgm2day~hydrology+season+i,data=ghg)
plot(ghg.lm)
anova(ghg.lm)
summary(ghg.lm)

t <- HSD.test(ghg.lm, "i", group=TRUE, alpha=0.01)
t
plot(t)

p <- ggplot(ghg, aes(x=season, y=co2cgm2day, color=season, shape=hydrology)) + 
  stat_summary(fun.data=mean_cl_boot,size=1.25) +  
  scale_color_manual(name="Season", values=colseason, labels = season) +     
  scale_shape_manual(name="Hydrology", values=c(17,16), labels = hydro_label) +      
  scale_x_discrete(labels=season,limits=c("sp", "sm", "f", "w")) + 
  ...
  ggtitle("Seasonal GHG Fluxes") +
  #set Tukey groups from HSD.test above
  annotate("text",x=1.0,y=0.65, label="b") +
  annotate("text",x=2.0,y=0.85, label="b") +
  annotate("text",x=3.0,y=0.62, label="b") +
  annotate("text",x=4.0,y=-0.2, label="b") +
  annotate("text",x=1.0,y=2.2, label="b") +
  annotate("text",x=2.0,y=6.55, label="a") + 
  annotate("text",x=3.0,y=2.0, label="b") + 
  annotate("text",x=4.0,y=1.8, label="b") + 
  #set plot design
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line =   
          element_line(colour = "black"),axis.title=element_text(size=12,face="bold"), 
...
#linear model for methane
i <- with(ghg, interaction(hydrology,season))
ghg.lm <- lm(n2ongm2day~hydrology+season+i,data=ghg)
plot(ghg.lm)
anova(ghg.lm)
summary(ghg.lm)
...
p <- ggplot(ghg, aes(x=season, y=n2ongm2day, color=season, shape=hydrology)) + 
  stat_summary(fun.data=mean_cl_boot,size=1.25) +  
  scale_color_manual(name="Season", values=colseason, labels = season) +     
  scale_shape_manual(name="Hydrology", values=c(17,16), labels = hydro_label) +      
  scale_x_discrete(labels=season,limits=c("sp", "sm", "f", "w")) + 
  ...
  annotate("text",x=2.25,y=0.0002, label="b") +
  annotate("text",x=3.25,y=-0.001, label="b") +
  annotate("text",x=4.25,y=-0.0005, label="b") +
  annotate("text",x=1.25,y=-0.0018, label="b") +
  annotate("text",x=2.25,y=-0.0045, label="a") + 
  annotate("text",x=3.25,y=-0.0027, label="b") + 
  annotate("text",x=4.25,y=-0.0014, label="b") + 
  #set plot design
  theme_bw() + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line =   
          element_line(colour = "black"),axis.title=element_text(size=12,face="bold"), 
...
p <- ggplot(ghg, aes(x=season, y=co2_ch4, color=season, shape=hydrology)) + 
  stat_summary(fun.data=mean_cl_boot,size=1.25) +  
  scale_color_manual(name="Season", values=colseason, labels = season) +     
  scale_shape_manual(name="Hydrology", values=c(17,16), labels = hydro_label) +      
  scale_x_discrete(labels=season,limits=c("sp", "sm", "f", "w")) + 
...
sharedfile = "C:/Users/annorion/GitHub/ConstructedStormwaterWetland/data/csw2017.opti_mcc_withArchaea.shared"
taxfile = "C:/Users/annorion/GitHub/ConstructedStormwaterWetland/data/csw2017.opti_mcc.0.03.cons_withArchaea.taxonomy"
metafile = "C:/Users/annorion/GitHub/ConstructedStormwaterWetland/data/CSW_16S_design.csv"
...
# Set treatments
treat1 <- as.factor(meta$season)
levels(treat1) <- ordered(c("may","oct"))
treat2 <- as.factor(meta$treatment)
levels(treat2) <- c("PI", "SL")
...
levels(pcoa.col) <- c("PI","SL")

#Run envfit to see which soil varibales are predicators 
#Read in meta file but trim to just may oct 1-5W,6t-9T,11T-13
...
plot1 <- ggplot(df1a, aes(x=V1, y=V2,  color=pcoa.shape, shape =  pcoa.col),
                 group = interaction(pcoa.col, pcoa.shape)) + theme_bw()  + 
  geom_point(aes(shape = pcoa.col, colour = pcoa.shape), size=7) + 
  ...
  xlab("PCoA 1 (17.4%)") + ylab("PCoA 2 (12.7%)") + 
  labs(shape = "Season") +
...
treat_merge = merge_samples(meta_merge, sample_data(meta_merge)$treatment)
...
ghg.lm <- lm(ch4co2eq~hydrology*season,data=ghg)
plot(ghg.lm)
anova(ghg.lm)

anova2<-(aov(ch4co2eq~as.factor(season)*as.factor(hydrology),data=ghg))
