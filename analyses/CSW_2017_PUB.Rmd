...
ghg <- mutate(ghg, 
      ...
      ch4co2eq=ghg$ch4cmgm2hr*25, #methane carbon dioxide equivalents
...
ghg$log.ch4 <- sign(ghg$ch4cmgm2hr) * log(1 - m + ghg$ch4cmgm2hr)
...
ch4.lm1 <- lmer(log.cb.ch4 ~ (1|sampleid) , data=ghg, REML=FALSE)

ch4.lm2 <- lmer(log.cb.ch4~hydrology + (1|sampleid), data=ghg, REML=TRUE)

ch4.lm3 <- lmer(log.cb.ch4~distance.bin + (1|sampleid), data=ghg, REML=FALSE)

ch4.lm4 <- lmer(log.cb.ch4~hydrology * distance.bin + (1|sampleid), data=ghg, REML=FALSE)

ch4.lm5 <- lmer(log.cb.ch4~season+ (1|sampleid), data=ghg, REML=FALSE)

ch4.lm6 <- lmer(log.cb.ch4~hydrology * season + (1|sampleid), data=ghg, REML=FALSE)

ch4.lm7 <- lmer(log.cb.ch4~distance.bin * season+ (1|sampleid), data=ghg, REML=FALSE)

ch4.lm8 <- lmer(log.cb.ch4~hydrology * distance.bin * season + (1|sampleid), data=ghg, REML=FALSE)
...
#require("multcomp")
#  ghg$hxs <- interaction(ghg$hydrology, ghg$season)
#  s <- glht(ch4.lm6, linfct=mcp(hxs='Tukey'))
#  summary(s)
...
#First I want to compare the randomness to parameters that I think might be influencing the response variable. In this case I want to know if hydrology, distance from the inlet, or season influence method production 
co2.lm1 <- lmer(log.cb.co2 ~ (1|sampleid) , data=ghg, REML=FALSE)

co2.lm2 <- lmer(log.cb.co2~hydrology + (1|sampleid), data=ghg, REML=TRUE)

co2.lm3 <- lmer(log.cb.co2~distance.bin + (1|sampleid), data=ghg, REML=FALSE)

co2.lm4 <- lmer(log.cb.co2~hydrology * distance.bin + (1|sampleid), data=ghg, REML=FALSE)

co2.lm5 <- lmer(log.cb.co2~season+ (1|sampleid), data=ghg, REML=FALSE)

co2.lm6 <- lmer(log.cb.co2~hydrology * season+ (1|sampleid), data=ghg, REML=FALSE)

co2.lm7 <- lmer(log.cb.co2~distance.bin * season+ (1|sampleid), data=ghg, REML=FALSE)

co2.lm8 <- lmer(log.cb.co2~hydrology * distance.bin * season + (1|sampleid), data=ghg, REML=FALSE)
...
#First I want to compare the randomness to parameters that I think might be influencing the response variable. In this case I want to know if hydrology, distance from the inlet, or season influence method production
n2on.lm1 <- lmer(sqrt.n2o ~ (1|sampleid) , data=ghg, REML=FALSE)

n2on.lm2 <- lmer(sqrt.n2o~hydrology + (1|sampleid), data=ghg, REML=TRUE)

n2on.lm3 <- lmer(sqrt.n2o~distance.bin + (1|sampleid), data=ghg, REML=FALSE)

n2on.lm4 <- lmer(sqrt.n2o~hydrology * distance.bin + (1|sampleid), data=ghg, REML=FALSE)

n2on.lm5 <- lmer(sqrt.n2o~season+ (1|sampleid), data=ghg, REML=FALSE)

n2on.lm6 <- lmer(sqrt.n2o~hydrology * season+ (1|sampleid), data=ghg, REML=FALSE)

n2on.lm7 <- lmer(sqrt.n2o~distance.bin * season+ (1|sampleid), data=ghg, REML=FALSE)

n2on.lm8 <- lmer(sqrt.n2o~hydrology * distance.bin * season + (1|sampleid), data=ghg, REML=FALSE)
...
n2on.lm1 <- lmer(log.cb.n2o ~ (1|sampleid) , data=ghg, REML=FALSE)

n2on.lm2 <- lmer(log.cb.n2o~hydrology + (1|sampleid), data=ghg, REML=TRUE)

n2on.lm3 <- lmer(log.cb.n2o~distance.bin + (1|sampleid), data=ghg, REML=FALSE)

n2on.lm4 <- lmer(log.cb.n2o~hydrology * distance.bin + (1|sampleid), data=ghg, REML=FALSE)

n2on.lm5 <- lmer(log.cb.n2o~season+ (1|sampleid), data=ghg, REML=FALSE)

n2on.lm6 <- lmer(log.cb.n2o~hydrology * season+ (1|sampleid), data=ghg, REML=FALSE)

n2on.lm7 <- lmer(log.cb.n2o~distance.bin * season+ (1|sampleid), data=ghg, REML=FALSE)

n2on.lm8 <- lmer(log.cb.n2o~hydrology * distance.bin * season + (1|sampleid), data=ghg, REML=FALSE)
...
#boxplots
p <- ggplot(ghg, aes(x=season, y=ch4cmgm2hr, color=season, shape=hydrology)) + geom_boxplot() +
  ...
  labs(x = "Season", y = (expression(paste("CH"[4], " (mg C"," m"^-{2}," hr"^-{1},")")))) +
...
p <- ggplot(ghg, aes(x=season, y=co2cmgm2hr, color=season, shape=hydrology)) + geom_boxplot() +
  geom_point(aes(color=factor(season)), size=2, position = position_jitterdodge()) +
  scale_color_manual(name="Season", values=colseason, labels = season_label) +     
  scale_shape_manual(name="Hydrology", values=c(17,21), labels = hydro_label) +      
  scale_x_discrete(labels=season_label,limits=c("sp", "sm", "f", "w")) + 
  labs(x = "Season", y = (expression(paste("CO"[2], " (mg C"," m"^-{2}," hr"^-{1},")")))) +
...
p <- ggplot(ghg, aes(x=season, y=n2onmgm2hr, color=season, shape=hydrology)) + geom_boxplot() +
  geom_point(aes(color=factor(season)), size=2, position = position_jitterdodge()) + 
  scale_color_manual(name="Season", values=colseason, labels = season_label) +     
  scale_shape_manual(name="Hydrology", values=c(17,21), labels = hydro_label) +      
  scale_x_discrete(labels=season_label,limits=c("sp", "sm", "f", "w")) + 
  labs(x = "Season", y = (expression(paste("N"[2],"O", " (mg N"," m"^-{2}," hr"^-{1},")")))) +
